...
RYA_estimate <- function(plant_counts=c(), 
  ...
  #             field_area: area of the field in ha
  ...
  if (precision_type %in% c("low","medium")) {
    ...
    #var_est <- ((dens_var/nrTri+avg_dens**2)*(RM_var/tot_plants + avg_RM**2)-(avg_dens*avg_RM)**2) *100
...
# test_plants <- list(rnorm(12, 1.5,0.7), rnorm(10,1.7,0.7), rnorm(9,1.7,0.7))
# 
...
test_c <- c(10,12,16)
test_rm <- rnorm(3*3, 1.5,0.7)
test_rm_new <- rnorm(6, 1.5,0.7)

json <- rjson::toJSON(test_c)

res <- RYA_estimate(plant_counts=test_c,plant_RMs=test_rm, precision_type="low", field_area=2)

json <- rjson::toJSON(res)

typeof(res)
typeof(json)
converted <- rjson::fromJSON(json)
typeof(converted)

plots <- RYA_plots(result=converted, field_area=2)
plots[1]
plots[2]


yieldperha <- res[3] ### is coming out per ha, if needed convert that to by acre
ifelse(yieldperha < 7.5, "low", 
       ifelse(yieldperha>=7.5 & yieldperha<15, "common", 
              ifelse(yieldperha >=15 & yieldperha<22.5, "high", "very high")))
